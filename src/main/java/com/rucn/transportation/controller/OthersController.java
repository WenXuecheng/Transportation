...
public class OthersController {
        ...
        //取us rmb汇率的值
        @GetMapping("/1/UsRmbGet/{id}")
        public Result findUsRmb(@PathVariable Integer id) {
                Others others = othersService.getById(id);
                return Result.success(others.getUsrmbRate());
        }
        //改us rmb汇率的值
        @GetMapping("/1/UsRmbSet/{value}")
        public Result changeUsRmb(@PathVariable Double value) {
                User user = TokenUtils.getCurrentUser();
                if (user.getRole().equals(RoleEnum.ROLE_USER.toString())){
                        throw new ServiceException(Constants.CODE_500, "权限认证失败");
                }
                othersMapper.setUsRmbRate(value);
                return Result.success();
        }
        //获取首页展示图片和公告
